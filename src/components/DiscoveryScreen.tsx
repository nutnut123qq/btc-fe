...
export function DiscoveryScreen() {
  ...
  const loadRules = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getDiscoveredRules();
      setRules(data);
    } catch (e) {
  ...
  const handleRun = async () => {
    ...
    setResult(null);
    try {
      const res = await runDiscovery("BTCUSDT", "1h", 2000, 5, 0.55, 15, 0.3, true);
      setResult(res);
      await loadRules();
    } catch (e) {
  ...
  const handleEvaluate = async () => {
    ...
    setEvalResult(null);
    try {
      const res = await evaluateSequenceRules("BTCUSDT", "1h", 50);
      setEvalResult(res);
    } catch (e) {
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      {evalResult && (
        <div className="bg-gray-900/60 rounded-xl border border-gray-800 p-4 text-xs space-y-2">
          ...
          {(evalResult.signals ?? []).map((s: any, i: number) => (
...
function DiscoveredRuleCard({ rule }: { rule: any }) {
